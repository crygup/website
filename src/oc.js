(function () {
  ...
  const sameRow = (a, b) =>
    Math.floor(a / GRID_SIZE) === Math.floor(b / GRID_SIZE);
  ...
  function possibleRedPositions(revealed) {
    ...
    for (const [pos, color] of Object.entries(revealed)) {
      ...
      for (const c of candidates) {
        ...
        else if (color === "yellow" && sameDiag(c, p) && !adjacent(c, p))
          filtered.add(c);
        else if (
          color === "green" &&
          (sameRow(c, p) || sameCol(c, p)) &&
          !sameDiag(c, p)
        )
          filtered.add(c);
        else if (
          color === "teal" &&
          (sameRow(c, p) || sameCol(c, p) || sameDiag(c, p))
        )
          filtered.add(c);
        else if (
          color === "blue" &&
  ...
  function bestNextClick(revealed) {
    ...
    for (const [color, count] of Object.entries(MAX_COUNTS)) {
      remaining[color] =
        count - Object.values(revealed).filter((c) => c === color).length;
    }
    ...
    const unrevealed = allPositions.filter(
      (p) => !(p in revealed) && p !== CENTER,
    );
    if (!unrevealed.length) return null;

    const redPos = Number(
      Object.entries(revealed).find(([, c]) => c === "red")?.[0],
    );
    ...
    if (redPos !== undefined && !isNaN(redPos)) {
      ...
      return unrevealed.reduce(
        (best, p) => (score(p) > score(best) ? p : best),
        unrevealed[0],
      );
    }
