(function () {
  ...
  function solve() {
    ...
    while (changed) {
      ...
      for (const [pos, color] of Object.entries(revealed)) {
        ...
        const unk = NEIGHBORS[p].filter(
          (n) => !(n in revealed) && !safe.has(n) && !mustBe.has(n),
        );
        const known = NEIGHBORS[p].filter(
          (n) => purples.has(n) || mustBe.has(n),
        ).length;
        if (known === num) {
          for (const n of unk) {
            safe.add(n);
      ...
      if (rem === 0) {
        ...
        unknown.clear();
      } else if (rem > 0 && unknown.size === rem) {
        for (const n of unknown) {
          mustBe.add(n);
          changed = true;
        }
  ...
  function bestClicks() {
    ...
    if (mustBe.size > 0) {
      return [...mustBe].sort(
        (a, b) =>
          NEIGHBORS[b].filter((x) => x in revealed).length -
          NEIGHBORS[a].filter((x) => x in revealed).length,
      );
    }
    const unrevealed = [];
    for (let i = 0; i < TOTAL; i++) {
      ...
      if (i in revealed) {
        ...
        if (useEmoji && EMOJI_IDS[revealed[i]]) {
          btn.style.setProperty(
            "--emoji",
            `url(https://cdn.discordapp.com/emojis/${EMOJI_IDS[revealed[i]]}.png)`,
          );
        }
      } else {
