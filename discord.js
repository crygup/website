...
function activeQuery() { return currentTab === "guild" ? guildQuery : userQuery; }
function setActiveQuery(v) { if (currentTab === "guild") guildQuery = v; else userQuery = v; }

const SUBTAB_LABELS = {
  avatars: "Avatars", usernames: "Usernames", "display-names": "Display Names",
  discrims: "Discrims", icons: "Icons", names: "Names",
};
...
document.getElementById("discord-tabs").addEventListener("click", (e) => {
  ...
  if (currentTab === "user") {
    ...
    currentSubtab = document.querySelector("#user-subtabs .subtab-btn.active")?.dataset?.subtab || "avatars";
    input.placeholder = "Discord ID or username…";
  } else if (currentTab === "guild") {
    guildSubtabs.classList.remove("hidden");
    currentSubtab = document.querySelector("#guild-subtabs .subtab-btn.active")?.dataset?.subtab || "icons";
...
userSubtabs.addEventListener("click", (e) => {
  ...
  if (!btn) return;
  userSubtabs.querySelectorAll(".subtab-btn").forEach(b => b.classList.remove("active"));
  btn.classList.add("active");
...
guildSubtabs.addEventListener("click", (e) => {
  const btn = e.target.closest(".subtab-btn");
  if (!btn) return;
  guildSubtabs.querySelectorAll(".subtab-btn").forEach(b => b.classList.remove("active"));
  btn.classList.add("active");
  currentSubtab = btn.dataset.subtab;
  currentPage = 1;
  updateDeleteAllLabel();
...
function showSettingsPanel() {
  if (!settingsPanel) { console.error("#settings-panel not found in DOM"); return; }
...
function renderSettings() {
  if (loggedInUser) {
    ...
    document.getElementById("settings-logout-btn").addEventListener("click", () => {
      ...
      loggedInUser = null; accessToken = null;
      hideSettingsPanel();
      tabs.forEach(b => b.classList.remove("active"));
      const userBtn = document.querySelector('#discord-tabs [data-tab="user"]');
      if (userBtn) userBtn.classList.add("active");
      currentTab = "user"; currentSubtab = "avatars";
      userSubtabs.classList.remove("hidden");
      renderLogin();
    });

    document.querySelectorAll("#settings-subtabs .subtab-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        else { document.getElementById("guild-select").value = ""; document.getElementById("guild-toggles").classList.add("hidden"); }
      });
    });
...
async function loadManagedGuilds() {
  if (!loggedInUser || !accessToken) { managedGuildIds = []; return; }
  try {
    const res = await fetch(`${FISHIE_API}/user/${loggedInUser.id}/guilds`, {
      headers: { "Authorization": `Bearer ${accessToken}` }
    });
...
async function saveGuildOptOuts(guildId) {
  ...
  const optedOut = GUILD_TRACKING_ITEMS
    .filter(item => {
      const input = document.querySelector(`#guild-toggles .guild-toggle[data-key="${item.key}"]`);
...
async function fetchOptOuts() {
  ...
  try {
    ...
    container.innerHTML = '<span class="toggle-status">Failed to load tracking settings.</span>';
  }
}
...
function renderToggles(optedOut) {
  ...
  container.querySelectorAll(".toggle-input:not([disabled])").forEach(input => {
...
async function saveOptOuts() {
  if (!loggedInUser || !accessToken) return;
  const optedOut = TRACKING_ITEMS
    .filter(item => !item.disabled)
    .filter(item => {
      const input = document.querySelector(`#tracking-toggles .toggle-input[data-key="${item.key}"]`);
      return input && !input.checked;
    })
    .map(item => item.key);
  try {
...
const GUILD_ENDPOINTS = {
  icons: (id, page) => `${FISHIE_API}/guild/${id}/icons?page=${page}&per_page=80`,
  names: (id, page) => `${FISHIE_API}/guild/${id}/names?page=${page}&per_page=80`,
};
const TABLE_MAP = {
  avatars: "avatars", usernames: "username_logs", "display-names": "display_name_logs",
  discrims: "discrim_logs", icons: "guild_icons", names: "guild_name_logs",
};

let managedGuildIds = [];

const canDelete = () => accessToken && loggedInUser && (
  (currentTab === "user" && currentUserId === String(loggedInUser.id)) ||
  (currentTab === "guild" && currentGuildId && managedGuildIds.includes(currentGuildId))
);
...
async function fetchData() {
  ...
  try {
    ...
    if (currentTab === "user" && !/^\d+$/.test(activeQuery())) {
      const r = await fetch(`${FISHIE_API}/resolve?q=${encodeURIComponent(activeQuery())}`);
...
function renderAvatars(avatars) {
  ...
  for (const av of avatars) {
    ...
    img.src = av.url || av.icon; img.alt = av.avatar_key || av.icon_key || ""; img.loading = "lazy";
    img.onerror = () => { img.src = ""; };
    div.appendChild(img);
...
function renderTextItems(items) {
  ...
  for (const item of items) {
    ...
    if (canDelete()) row.querySelector(".delete-btn").addEventListener("click", e => { e.stopPropagation(); deleteItem(TABLE_MAP[currentSubtab], key); });
...
async function deleteItem(table, key) {
  ...
  try {
    const res = await fetch(`${FISHIE_API}/item/${table}/${targetId}?key=${encodeURIComponent(key)}`, { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } });
...
async function deleteAll() {
  ...
  const targetId = currentTab === "user" ? currentUserId : currentGuildId;
  try {
    const res = await fetch(`${FISHIE_API}/user/${targetId}?table=${TABLE_MAP[currentSubtab]}`, { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } });
...
if (subtabParam) {
  const subtabBar = currentTab === "user" ? userSubtabs : currentTab === "guild" ? guildSubtabs : null;
  if (subtabBar) {
    const subtabBtn = subtabBar.querySelector(`[data-subtab="${subtabParam}"]`);
    if (subtabBtn) {
      subtabBar.querySelectorAll(".subtab-btn").forEach(b => b.classList.remove("active"));
...
const wantsSettings = currentTab === "settings" || localStorage.getItem("settings_pending");
if (wantsSettings) {
  document.querySelectorAll("#discord-tabs .tab-btn").forEach(b => b.classList.remove("active"));
  const settingsTab = document.querySelector('#discord-tabs [data-tab="settings"]');
