...
if (code) {
  (async () => {
    try {
      const res = await fetch(API + "/oauth/exchange?code=" + code);
...
if (_logout) _logout.onclick = function() { localStorage.removeItem("fishie_token"); localStorage.removeItem("fishie_user"); location.reload(); };
var _login = document.getElementById("loginBtn");
if (_login) _login.onclick = function() {
  window.location.href = "https://discord.com/api/oauth2/authorize?client_id=" + CLIENT_ID + "&redirect_uri=" + encodeURIComponent(REDIRECT) + "&response_type=code&scope=identify%20guilds";
};
...
async function initDashboard() {
  ...
  try {
    ...
    if (fcData.first_command) {
      ...
      document.getElementById("userSince").textContent = "Fishie user since " + d.toISOString().split("T")[0];
    }
  } catch (_) {}
...
async function loadUserSettings(userId, token) {
  ...
  try {
    var [optRes, remRes, accRes, xpRes] = await Promise.all([
      fetch(API + "/user/" + userId + "/opted-out", { headers: {"Authorization":"Bearer " + token} }),
      fetch(API + "/user/" + userId + "/reminders", { headers: {"Authorization":"Bearer " + token} }),
      fetch(API + "/user/" + userId + "/accounts", { headers: {"Authorization":"Bearer " + token} }),
      fetch(API + "/user/" + userId + "/xp", { headers: {"Authorization":"Bearer " + token} })
    ]);
    ...
    var items = [
      {k:"avatar",l:"Avatar"},{k:"username",l:"Username"},{k:"display",l:"Display name"},
      {k:"nickname",l:"Nickname"},{k:"discrim",l:"Discriminator"},{k:"joins",l:"Server joins"}
    ];

    var html = '<div class="card"><div class="settings-group"><h4>Tracking</h4>';
    for (var i = 0; i < items.length; i++) {
      var on = !optedOut.has(items[i].k);
      html += '<div class="setting-toggle"><div class="label">' + items[i].l + '</div>'
        + '<div class="toggle ' + (on ? "on" : "") + '" data-optout="' + items[i].k
        + '" onclick="var t=this;t.classList.toggle(\'on\');togUserOpt(\'' + userId + '\',\'' + items[i].k + '\',t.classList.contains(\'on\'))"></div></div>';
    }
    html += '</div></div>';

    html += '<div class="card"><div class="settings-group"><h4>XP</h4>'
      + '<div style="display:flex;gap:1.5rem"><div><div class="label">Messages</div><div class="value">' + (xpData.messages||0).toLocaleString() + '</div></div>'
      + '<div><div class="label">XP</div><div class="value">' + (xpData.xp||0).toLocaleString() + '</div></div></div></div></div>';

    html += '<div class="card"><div class="settings-group"><h4>Reminders</h4>';
    var rems = remData.reminders || [];
    if (!rems.length) {
      html += '<div style="color:#64748b;font-size:0.8rem">No active reminders</div>';
    } else {
      for (var ri = 0; ri < rems.length; ri++) {
        ...
        var timeLeft = diff === 0 ? "Expired" : (days ? days + "d " : "") + hours + "h " + mins + "m";
        html += '<div class="row"><span style="font-size:0.82rem;color:#cbd5e1">' + esc(r.content||"Reminder") + '</span><span style="color:#64748b;font-size:0.75rem">' + timeLeft + '</span></div>';
      }
    }
    html += '</div></div>';

    html += '<div class="card"><div class="settings-group"><h4>Connected Accounts</h4>';
    var accts = accData.accounts || {};
    var svcs = ["lastfm","steam","osu","roblox","genshin","letterboxd"];
    for (var si = 0; si < svcs.length; si++) {
      var svc = svcs[si];
      var val = accts[svc] || "";
      html += '<div class="row" style="display:flex;align-items:center;gap:0.3rem;flex-wrap:wrap">'
        + '<span style="color:#94a3b8;font-size:0.78rem;min-width:60px">' + svc + '</span>'
        + '<input type="text" id="acct-' + svc + '" placeholder="' + svc + ' username" value="' + esc(val) + '" class="text-input" style="flex:1;min-width:100px"></div>';
    }
    html += '<button class="btn-primary" style="margin-top:0.5rem" onclick="saveAllAccounts(' + "'" + userId + "'" + ')">Save Accounts</button>';
    html += '</div></div>';
...
async function togUserOpt(userId, item, enable) {
  var token = localStorage.getItem("fishie_token");
  try {
    var res = await fetch(API + "/user/" + userId + "/opted-out", { headers: { "Authorization": "Bearer " + token } });
    var data = await res.json();
    var items = data.items || [];
    if (enable) items = items.filter(function(i) { return i !== item; });
    else { if (!items.includes(item)) items.push(item); }
...
async function loadGuilds(userId, token) {
  try {
    ...
    list.innerHTML = '<div class="guild-dropdown-item" data-id="">'
    ...
    for (var gi = 0; gi < guilds.length; gi++) {
      ...
      var img = iconUrl ? '<img src="' + iconUrl + '" alt="" loading="lazy">' : "";
      list.innerHTML += '<div class="guild-dropdown-item" data-id="' + g.id + '">'
        + img + '<div class="item-info">'
        + '<div class="item-name">' + esc(g.name) + '</div>'
        + '<div class="item-sub">ID: ' + g.id + '</div>'
        + '</div></div>';
    }
    if (list.dataset.selectedId) {
      var prev = list.querySelector('[data-id="' + list.dataset.selectedId + '"]');
      if (prev) text.textContent = prev.querySelector(".item-name").textContent;
    }
  } catch (_) {
    document.getElementById("guildDropdownList").innerHTML = '<div style="color:#64748b;padding:0.5rem;font-size:0.85rem">Failed to load servers</div>';
  }
}

function selectGuild(guildId) {
  if (!guildId) { document.getElementById("guildSettingsContent").innerHTML = ""; return; }
...
async function loadGuildSettings(guildId, token) {
  ...
  try {
    ...
    var [gRes, setRes, optRes, preRes] = await Promise.all([
      ...
      fetch(API + "/guild/" + guildId + "/settings", { headers: { "Authorization": "Bearer " + token } }),
    ...
    for (var gi = 0; gi < (gData.guilds || []).length; gi++) {
      if (gData.guilds[gi].id === guildId) { guild = gData.guilds[gi]; break; }
    }
    ...
    if (prefixes.length === 0) {
      html += '<span style="color:#64748b;font-size:0.8rem">No custom prefixes</span>';
    } else {
      for (var pi = 0; pi < prefixes.length; pi++) {
        html += '<span class="prefix-tag">' + esc(prefixes[pi].prefix) + ' <span class="remove" onclick="remPrefix(' + "'" + guildId + "'" + ',\'' + esc(prefixes[pi].prefix) + '\')">×</span></span>';
      }
    }
    html += '</div>';
    ...
    for (var ti = 0; ti < tItems.length; ti++) {
      ...
      html += '<div class="setting-toggle"><div class="label">' + tItems[ti].l + '</div>'
        + '<div class="toggle ' + (on ? "on" : "") + '" onclick="var t=this;t.classList.toggle(\'on\');togGOpt(' + "'" + guildId + "'" + ',\'' + tItems[ti].k + '\',t.classList.contains(\'on\'))"></div></div>';
    }
    html += '</div>';
    ...
    div.innerHTML = '<p style="color:#64748b">Failed to load guild settings.</p>';
  }
}
